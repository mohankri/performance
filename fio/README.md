...
linux scheduler will have impact on this number.
Check /sys/block/sd[x]/queue/scheduler
```

```
Completion Latency (Time for the IO to complete, doesn't include Submission Latency)
==================
...
```
Application Level Latency (This the latency application will notices 
=========================
...
```
Bandwidth (50% of the total IO served by the devices... need to make more sense out of it..)
=========
bw (KB  /s): min=19792, max=26336, per=50.01%, avg=24581.48, stdev=500.22
...
===================
lat (usec) : 2=0.01%, 4=0.01%, 10=0.01%, 20=0.01%, 50=49.88%  (49.88% of the request took less 50usec)
lat (usec) : 100=0.08%, 250=49.76%, 500=0.21%, 750=0.02%, 1000=0.01%  (0.08% of the request took lest 100usec)
lat (msec) : 2=0.02%, 4=0.01%, 10=0.01%, 50=0.01%, 100=0.01% 
(read like this 2000 usec, 4000usec, 10,000usec, 50,000usec, 100,000usec or 0.02% of the request took 2000usec or 2msec)
...
majf: major fault
minf: minor fault  (Direct IO causes Less Page fault)
...
IO Depth
========
How many IOs are issued to OS at any given time. It's not the device IO depth queue.
In our example IOdepth is 1 that's why it's 100% and rest are 0%

submit/complete show 4=100% as IOdepth=1 falls into bucket[1-4] i.e 4=100%
issued: shows total number read, write IO issued.
...
Status
======
 This an aggregate result across all the device
